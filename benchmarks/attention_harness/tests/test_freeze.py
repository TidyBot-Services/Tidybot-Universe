...
def test_repository_manifest_is_deterministic_and_blocks_heldout(tmp_path) -> None:
    first = create_manifest()
    second = create_manifest()
    assert first == second
    assert first["d6_parity"]["agreement"] == 1.0
    assert first["heldout_ready"] is False
    manifest = tmp_path / "manifest.json"
    _write(manifest, first)
    assert verify_manifest(manifest)["verified"] is True
    with pytest.raises(FreezeError, match="held-out run denied"):
        verify_manifest(manifest, require_heldout_ready=True)
